function customerprompt(variant=null,forwardlink=null,backwardlink=null){
    ...
    if(variant == 'confirmdelivery'){
        ...
        <div class="buttons">
            <button onclick="customerprompt();customerprompt('thankyou','https://localhost/mvc');">
            ...
            <button onclick="customerprompt();displaychargeandaddress();">
...
function displaychargeandaddress(){
    ...
    xhr.open('POST', 'http://localhost/mvc/delchargeandaddress/getboth', true);
    xhr.onload = ()=>{
        if(xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200){
            let data = xhr.response;
            if(data){
                // now we have the default address and the relevent charge
                customerprompt('deliverychargeandaddress','/Orders/getcollecting_method/Pickup/','/Orders/getcollecting_method/Pickup/');
            }
        }
    }
    xhr.send();
